...
// `settingHelpers`. Kept under the old names here so no presenting flow call site has
...
export function toPresentingFlowSettingKey(...parts: string[]) {
    return toFilePathSettingKey(...parts);
}

export function toPresentingFlowSettingName(
    prefix: string,
    ...parts: string[]
) {
...
/**
 * Every prefix a presenting flow persists under. Listed here rather than at each use
 * site so the cleanup below cannot miss one that is added later — the whole
 * point of it is that nothing is left behind.
 */
const PRESENTING_FLOW_SETTING_PREFIXES = [
...
/**
 * Forget everything a presenting flow file persisted, once that file is gone.
 *
 * A presenting flow's settings are named after its path, so a deleted presenting flow used to
 * leave one file behind per setting it had ever written — including one
 * `presenting-flow-item-expanded-…` per document it held — with nothing left that could
 * ever read or clear them again.
 */
export async function removePresentingFlowSettings(filePath: string) {
    const key = toPresentingFlowSettingKey(filePath);
    const removedKeys = await Promise.all(
        PRESENTING_FLOW_SETTING_PREFIXES.map((prefix) => {
 ...
 * in a presenting flow that answers to both.
 *
 * `position` puts what is being dragged AT that line: an existing row of this
 * presenting flow MOVES there, anything else is INSERTED there. `cc` attaches it to
 * that line instead.
 *
 * WHERE on the row the pointer is decides by default, but only for a row of this
 * presenting flow: its EDGES take the position, its MIDDLE attaches. The band is fixed
 ...
 */
export function toPresentingFlowRowDropKind(
    event: {
        ctrlKey?: boolean;
        metaKey?: boolean;
        altKey?: boolean;
...
/**
 * Set while a row is dragged out of a presenting flow. A drop back into the SAME
 * presenting flow is then a reorder rather than an add — the dragged payload alone
 * cannot tell the two apart.
 */
export const presentingFlowDraggingStore: {
    current: { filePath: string; index: number } | null;
} = { current: null };

/**
 * A presenting flow needs both halves of a drag: the deserialized item (to read a
 ...
 *   handling for those.
 * - `'unsupported'` — it WAS an app payload, of a type a presenting flow cannot take.
 ...
 *   (`PresentingFlowItem.fromDroppedData` returning null) and was unreachable, because
...
/**
 * Drop a presenting flow row onto a screen previewer. Slides resolve asynchronously, so
 ...
 */
export async function handlePresentingFlowItemScreenDropping(
...
/**
 * The one way a presenting flow element is FIRED: content is shown on the screens, a
 ...
 * once at the top. `armPresentingFlowCcPropagation` parks a closure in a module-level
 ...
 */
export async function sendPresentingFlowItemToScreens(
    presentingFlowItem: PresentingFlowItem,
    event: any,
    ...
    if (presentingFlowItem.isRunAction) {
        const ccItems = presentingFlowItem.hostsCcFollowers
            ? presentingFlowItem.ccItems
            : [];
        if (ccItems.length === 0) {
        ...
        armPresentingFlowCcPropagation(event, ccItems);
    ...
    const { screenIds } = presentingFlowItem;
    const screenAction = presentingFlowItem.screenAction;
    if (screenAction !== null) {
        ...
        armPresentingFlowCcPropagation(event, presentingFlowItem.ccItems);
    ...
    if (droppedData === null) {
        ...
        showSimpleToast(
            tran('Showing Presenting Flow Item'),
