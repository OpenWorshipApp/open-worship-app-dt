...
} = await import('./presentingFlowActionKeyHelpers');
...
} = await import('./presentingFlowPreviewFloatingHelpers');
...
/** The file layer stubbed on the instance, as the other presenting flow tests do. */
function genPresentingFlow(items: any[]) {
    instanceCount += 1;
    const presentingFlow = PresentingFlow.getInstance(
        `/presenting-flows/key-${instanceCount}.owpf`,
    );
    ...
    (presentingFlow as any).save = vi.fn(async () => {
        return true;
    });
...
describe('the stored shortcut format', () => {
    test('it is a modifier and a key, in one spelling', () => {
        expect(checkIsValidPresentingFlowActionKey('Shift+A')).toBe(true);
        expect(checkIsValidPresentingFlowActionKey('Ctrl+A')).toBe(true);
        expect(checkIsValidPresentingFlowActionKey('Ctrl+Shift+A')).toBe(true);
        expect(checkIsValidPresentingFlowActionKey('Ctrl+Shift+F5')).toBe(true);

        // A bare key would take that key away from the run player itself.
        expect(checkIsValidPresentingFlowActionKey('A')).toBe(false);
        // One shortcut, one spelling — otherwise two lines could both claim it
        // while `PresentingFlow`'s uniqueness check saw two different strings.
        expect(checkIsValidPresentingFlowActionKey('Shift+Ctrl+A')).toBe(false);
        expect(checkIsValidPresentingFlowActionKey('Shift+a')).toBe(false);
        // Not portable, so never stored.
        expect(checkIsValidPresentingFlowActionKey('Alt+A')).toBe(false);
        expect(checkIsValidPresentingFlowActionKey('Meta+A')).toBe(false);
        expect(checkIsValidPresentingFlowActionKey('Shift+Shift+A')).toBe(
            false,
        );
        expect(checkIsValidPresentingFlowActionKey('')).toBe(false);
        expect(checkIsValidPresentingFlowActionKey(undefined)).toBe(false);
        expect(checkIsValidPresentingFlowActionKey(7)).toBe(false);
    });
    ...
    test('it speaks the app’s keyboard layer through allControlKey', () => {
        ...
        expect(toPresentingFlowActionKeyEventMapper('Shift+ArrowUp')).toEqual({
            key: 'ArrowUp',
            allControlKey: ['Shift'],
        });
        expect(toPresentingFlowActionKeyEventMapper('A')).toBeNull();
    });
});
...
describe('reading a shortcut off a key press', () => {
    ...
    test('a press that cannot be one says which', () => {
        expect(readPresentingFlowActionKeyFromEvent(genKeyEvent({}))).toEqual({
            actionKey: null,
...
describe('a Keyboard Event element', () => {
    test('it reads as its shortcut and hosts followers, not targets', () => {
        ...
        expect(() => {
            PresentingFlowItem.validate(unarmed.toJson() as any);
        }).not.toThrow();
    });
...
describe('a shortcut is unique in its presenting flow', () => {
    ...
    test('re-arming keeps its own key but cannot take another line’s', async () => {
        ...
        expect(
            await presentingFlow.setItemActionArming(0, {
                actionKey: 'Shift+A',
            }),
        ).toBe(true);
        expect(
    ...
    test('the three armings are alternatives, so one replaces the others', async () => {
        ...
        await presentingFlow.setItemActionArming(0, { actionKey: 'Shift+A' });
    ...
    test('a duplicate is left unarmed rather than claiming the same key', async () => {
        ...
        expect(await presentingFlow.duplicateItemAtIndex(0)).toBe(true);
...
describe('what the run registers', () => {
    test('armed lines only, deduped, first one winning', () => {
        ...
        expect(
            collectPresentingFlowRunShortcutKeys(presentingFlowItems),
        ).toEqual(['Shift+A', 'Ctrl+B']);
        expect(
            findPresentingFlowRunShortcutUuid(presentingFlowItems, 'Shift+A'),
        ).toBe('u-1');
        expect(
            findPresentingFlowRunShortcutUuid(presentingFlowItems, 'Ctrl+B'),
        ).toBe('u-3');
        expect(
            findPresentingFlowRunShortcutUuid(presentingFlowItems, 'Shift+C'),
        ).toBeNull();
        expect(
            findPresentingFlowRunShortcutUuid(presentingFlowItems, 'Shift+Z'),
        ).toBeNull();
