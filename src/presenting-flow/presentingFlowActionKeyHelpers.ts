 ...
 * Mac) — a presenting flow file is opened on whatever machine is in the building, so
 ...
 *
 * Pure, and free of every UI import: `PresentingFlowItem` (which draws the row and
...
type PresentingFlowActionKeyPartsType = {
 ...
 */
function toPresentingFlowActionKeyParts(
    value: any,
): PresentingFlowActionKeyPartsType | null {
...
export function checkIsValidPresentingFlowActionKey(
    value: any,
): value is string {
    return toPresentingFlowActionKeyParts(value) !== null;
}
 ...
 */
export function toPresentingFlowActionKeyEventMapper(
    value: any,
): EventMapperType | null {
    const parts = toPresentingFlowActionKeyParts(value);
    if (parts === null) {
        return null;
    }
 ...
 */
export type PresentingFlowActionKeyAnswerType =
    { actionKey: string; message: null } | { actionKey: null; message: string };

export function readPresentingFlowActionKeyFromEvent(
    event: KeyboardEvent | { key: string; code?: string } | any,
): PresentingFlowActionKeyAnswerType | null {
    ...
    if (!checkIsValidPresentingFlowActionKey(actionKey)) {
