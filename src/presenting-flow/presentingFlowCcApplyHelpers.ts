 ...
 *
 * Its own module, and deliberately importing `PresentingFlowItem` as a TYPE only:
 * `presentingFlowHelpers` calls into this and `PresentingFlowItem` imports
 * `presentingFlowHelpers`, so a value import here would close the cycle
 * `presentingFlowItemsHelpers` was split out to avoid.
 */
 ...
 * `PresentingFlowItem` strips the flag on write and on read alike — so a listed CC is
 * one that fires. What cannot reach a screen at all is still stepped over.
 */
export async function applyPresentingFlowCcItemsOnScreenIds(
    ...
    for (const ccItem of ccItems) {
        ...
        if (runAction !== null) {
            if (runAction.canBeCcItem) {
                firePresentingFlowRunAction(ccItem);
            }
            continue;
        }
    ...
    // splitting. The on-screen helpers import `PresentingFlow` and run screen-manager
    // subscriptions at module scope, and this module sits under
    // `presentingFlowHelpers`, which `PresentingFlowItem` imports: a static import here
    // would close that cycle and drag the whole screen graph into merely
    // LISTING a presenting flow. The build therefore reports this as an
    ...
    const { refreshOnScreenAfterPresenting } =
        await import('./presentingFlowOnScreenHelpers');
 ...
 */
export function armPresentingFlowCcPropagation(
    event: any,
    ccItems: PresentingFlowItem[],
) {
    if (ccItems.length === 0) {
    ...
    captureChosenScreenIds(event, (screenIds) => {
        ...
        applyPresentingFlowCcItemsOnScreenIds(ccItems, screenIds).catch(
            handleError,
        );
    });
}
