 ...
 *
 * Pure, and free of every UI import, because `PresentingFlowItem` (which draws the
 * row and validates the entry) and `presentingFlowAutoNextHelpers` (which counts down
 * to it) both need it.
 */
...
export function checkIsValidPresentingFlowActionTime(
    value: any,
): value is string {
 ...
 */
export function toPresentingFlowActionTimeLabel(time: string) {
    if (!checkIsValidPresentingFlowActionTime(time)) {
        return time;
    }
...
export function toPresentingFlowActionTime(date: Date) {
 ...
 */
export function toPresentingFlowActionDueTime(
    time: string,
    fromTime: number,
): number | null {
    if (!checkIsValidPresentingFlowActionTime(time)) {
