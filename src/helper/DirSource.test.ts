...
describe('DirSource', () => {
    ...
    test('alertFileChanging tells the changed file itself', async () => {
        const settingName = genSettingName();
        const dirSource = new DirSource(settingName);
        const fireUpdateEvent = vi.fn();
        fileSourceGetInstanceMock.mockReturnValue({ fireUpdateEvent });

        // Without a dir path there is no file to name, so nothing is told.
        await dirSource.alertFileChanging('a.owa');
        expect(fileSourceGetInstanceMock).not.toHaveBeenCalled();
        ...
        expect(fireUpdateEvent).toHaveBeenCalledTimes(1);
        // An ordinary file is not a sidecar of anything, so the filesystem is
        // never asked and the file is never told twice.
        expect(fsCheckFileExistMock).not.toHaveBeenCalled();
    });

    test('alertFileChanging blames the file a history folder belongs to', async () => {
        const settingName = genSettingName();
        settingStore.set(settingName, '/docs');
        const dirSource = new DirSource(settingName);
        ...
        await dirSource.alertFileChanging('a.owa.histories/12');

        expect(fsCheckFileExistMock).toHaveBeenCalledWith('/docs/a.owa');
        expect(firedPaths).toEqual(['/docs/a.owa.histories/12', '/docs/a.owa']);

        // A history folder whose document is gone names nothing extra.
        firedPaths.length = 0;
        fsCheckFileExistMock.mockResolvedValue(false);
        await dirSource.alertFileChanging('b.owa.histories/3');

        expect(firedPaths).toEqual(['/docs/b.owa.histories/3']);
    });
});
