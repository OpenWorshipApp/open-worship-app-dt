...
describe('DirSource', () => {
    ...
    test('alertFileChanging emits the changed file path', async () => {
        const settingName = genSettingName();
        const dirSource = new DirSource(settingName);
        const changedPaths: string[] = [];
        dirSource.registerEventListener<string>(['file-update'], (filePath) => {
        ...
        expect(changedPaths).toEqual(['/docs/a.owa']);
    });
});
