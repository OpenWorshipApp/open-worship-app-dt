...
// A TYPE only: `PlaylistItem` imports this module for its values, so anything
 ...
 * `playlistActionList`: storage, validation, the row, the preview, the drag and
...
type PlaylistActionBaseType = {
     ...
     * family (`PlaylistItem.getMaxCcItemCount`).
     *
...
export type PlaylistScreenActionType = PlaylistActionBaseType & {
    target: 'screen';
    /**
     * Whether it may only ever run on the screens it NAMES — its pin
     * (`PlaylistItem.screenIds`) — rather than falling back to the selected
...
export type PlaylistRunActionType = PlaylistActionBaseType & {
    target: 'run';
    /**
     ...
     * everywhere `PlaylistActionArmingType` is.
     */
     ...
     * `PlaylistItem.checkIsCcTargetHost`, and through it by every attach path.
     */
     ...
     */
    start: (playlistItem: PlaylistItem) => string | null;
};

export type PlaylistActionType =
    PlaylistScreenActionType | PlaylistRunActionType;
 ...
 * others (`Playlist.setItemActionArming`) or a re-armed line would still be
 * carrying the answer it was given last time.
 */
export type PlaylistActionArmingType = {
    actionNumber?: number;
    actionTime?: string;
    /**
     * The third alternative, and the one that is not a clock: the SHORTCUT that
     * fires this line. `Ctrl+Shift+A` — see `playlistActionKeyHelpers` for the
 ...
 */
const foregroundClearActionList: PlaylistScreenActionType[] = Object.entries(
    foregroundClearMap,
).map(([target, { label, badge, clear }]): PlaylistScreenActionType => {
    return {
 ...
 * at a time of day (see `playlistAutoNextHelpers`) — could fill a screen and
 ...
 */
const screenShowHideActionList: PlaylistScreenActionType[] = [
    {
        id: 'screen-show',
        label: 'Screen: Show',
        badge: 'ON',
 ...
 * `playlistAutoNextHelpers` for why input cancels one and not the other — so they
...
const NO_RUN_MESSAGE = 'Open the playlist preview to use this action';

const nextActionList: PlaylistRunActionType[] = [
    {
        id: 'next-interval',
        label: 'Next: Interval',
        badge: '⟳',
        ...
        start: (playlistItem) => {
            return startPlaylistAutoNext(
                playlistItem.filePath,
                'interval',
                playlistItem.actionNumber,
            )
                ? null
                : NO_RUN_MESSAGE;
        },
    },
    {
        ...
        start: (playlistItem) => {
            ...
            const { actionTime } = playlistItem;
            if (actionTime !== null) {
                return startPlaylistAutoNextAtTime(
                    playlistItem.filePath,
                    actionTime,
                );
            }
    ...
    },
    {
         ...
         *
         * Where it goes is named the way everything else in a playlist names
        ...
        movesRunAtOnce: true,
        start: (playlistItem) => {
            const [ccItem] = playlistItem.ccItems;
            if (ccItem === undefined) {
                return 'Attach the element to jump to as a CC element';
            }
            return jumpPlaylistRunToCcItem(playlistItem.filePath, ccItem);
        },
    },
    {
        /**
         ...
         * by `sendPlaylistItemToScreens`, which is where the screens are resolved
         ...
         */
        start: (playlistItem) => {
            return playlistItem.ccItems.length === 0
 ...
 */
const clearActionList: PlaylistScreenActionType[] = [
    {
        id: 'clear-all',
        label: 'Clear All',
        badge: 'ALL',
 ...
 */
export type PlaylistActionGroupType = {
    label: string;
    iconName: string;
    color: string;
    actionList: PlaylistActionType[];
};

export type PlaylistActionMenuEntryType =
    PlaylistActionType | PlaylistActionGroupType;

export function checkIsPlaylistActionGroup(
    entry: PlaylistActionMenuEntryType,
): entry is PlaylistActionGroupType {
    return (entry as PlaylistActionGroupType).actionList !== undefined;
}
 ...
 */
export const playlistActionMenuList: PlaylistActionMenuEntryType[] = [
    ...clearActionList,
    {
 ...
 * `playlistActionMenuList`; nothing else may depend on it.
 */
