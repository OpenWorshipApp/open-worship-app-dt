...
import type {
    ...
    PlaylistActionIdType,
    PlaylistRunActionType,
    PlaylistScreenActionType,
} from './playlistActionHelpers';
import {
    PLAYLIST_ACTION_TYPE,
    findPlaylistAction,
} from './playlistActionHelpers';
import { checkIsValidPlaylistActionKey } from './playlistActionKeyHelpers';
import {
    checkIsValidPlaylistActionTime,
    toPlaylistActionTimeLabel,
} from './playlistActionTimeHelpers';
import type { PlaylistCcItemType } from './playlistCcHelpers';
import {
    checkIsValidPlaylistItemUuid,
    genPlaylistItemUuid,
...
export type PlaylistItemType = {
    ...
    // on add and never rewritten; see `genPlaylistItemUuid`. Optional in the type
    // for the one moment it is absent — a file written before uuids existed,
    // which `Playlist.getJsonData` fills in on read.
    uuid?: string;
    filePath?: string;
    id?: number;
    ...
    // UNIQUE within the playlist — one shortcut names one line, enforced by
    // `Playlist` on every write. See `playlistActionKeyHelpers`.
    actionKey?: string;
    ...
    // entry by its uuid (see `PlaylistCcItemType`). A reference and not a copy,
    ...
    slideCcItems?: { [slideId: string]: PlaylistCcItemType[] };
};
...
export default class PlaylistItem {
    ...
    private ccItemsCache: PlaylistItem[] | null = null;
    private slideCcItemsCache: Map<number, PlaylistItem[]> | null = null;
    ...
    private ccSourceMap: ReadonlyMap<string, PlaylistItem> | null = null;
     ...
     * `Playlist.getItems`, right where the whole list exists and nowhere else.
     *
     ...
     */
    static bindCcSources(playlistItems: PlaylistItem[]) {
        const hasAnyCcItem = playlistItems.some((playlistItem) => {
            return playlistItem.hasCcItems || playlistItem.hasSlideCcItems;
        });
        if (!hasAnyCcItem) {
            return playlistItems;
        }
        const ccSourceMap = new Map<string, PlaylistItem>();
        for (const playlistItem of playlistItems) {
            ...
            if (uuid !== null && !ccSourceMap.has(uuid)) {
                ccSourceMap.set(uuid, playlistItem);
            }
        }
        for (const playlistItem of playlistItems) {
            playlistItem.ccSourceMap = ccSourceMap;
        }
        return playlistItems;
    }
    ...
    get uuid(): string | null {
        ...
        return checkIsValidPlaylistItemUuid(uuid) ? uuid : null;
    }
     ...
     * its own to ride.
     */
    static getMaxCcItemCount(itemJson: PlaylistItemType) {
        if (itemJson.type === ERROR_TYPE) {
            return 0;
        }
        if (itemJson.type !== PLAYLIST_ACTION_TYPE) {
            return Infinity;
        }
        return findPlaylistAction(itemJson.data)?.ccItemCount ?? 0;
    }

    get maxCcItemCount() {
        return PlaylistItem.getMaxCcItemCount(this.originalJson);
    }
     ...
     * unlike a bad entry, which `Playlist.getItems` turns into an error row. That
     ...
     * and in the file (`Playlist.removeItemAtIndex`).
     *
     ...
     */
    private buildCcItems(rawList: any, isTarget: boolean): PlaylistItem[] {
        ...
        for (const raw of rawList) {
            ...
            const ccJson = PlaylistItem.resolveCcItemJson(
                sourceItem.toJson(),
                isTarget,
            );
            if (ccJson === null) {
            ...
            items.push(new PlaylistItem(this.filePath, ccJson));
        }
        return items;
    }
     ...
     */
    getEffectiveSlideCcItems(slideId: number): PlaylistItem[] {
        if (!this.hasCcItems) {
     ...
     */
    get screenAction(): PlaylistScreenActionType | null {
        const { action } = this;
        return action?.target === 'screen' ? action : null;
    }
    get runAction(): PlaylistRunActionType | null {
        const { action } = this;
    ...
    get actionTime(): string | null {
        ...
        return checkIsValidPlaylistActionTime(actionTime) ? actionTime : null;
    }
    /**
    ...
    get actionKey(): string | null {
        ...
        return checkIsValidPlaylistActionKey(actionKey) ? actionKey : null;
    }
    /** How this run action is armed, as the question asks and answers it. */
    get actionArming(): PlaylistActionArmingType {
    ...
    get title() {
        ...
        if (this.isAction) {
            ...
            const arming =
                actionTime !== null
                    ? toPlaylistActionTimeLabel(actionTime)
                    : `${this.actionNumber}`;
     ...
     * app's language and what the action does lives in code, so a playlist
     ...
     * absence of the key everywhere else (`Playlist.setItemScreenIds`), so a
     * `Screen: Show` re-aimed from the row's own `Set Specific Screen` and one
     * added from the menu leave identical json.
     */
    static fromActionId(
        actionId: PlaylistActionIdType,
        arming: PlaylistActionArmingType = {},
        screenIds: number[] = [],
    ): PlaylistItemType {
        return {
     ...
     */
    static checkIsCcTargetHost(hostItemJson: PlaylistItemType) {
        if (hostItemJson.type !== PLAYLIST_ACTION_TYPE) {
            return false;
        }
        const action = findPlaylistAction(hostItemJson.data);
    ...
    static resolveCcItemJson(
        itemJson: PlaylistItemType,
        isTarget = false,
    ): PlaylistItemType | null {
        const {
            ccItems: _ccItems,
            ...
            ...ccJson
        } = cloneJson(itemJson) as PlaylistItemType;
        ...
        if (ccJson.type === PLAYLIST_ACTION_TYPE) {
            const action = findPlaylistAction(ccJson.data);
     ...
     * caller that rewrites one rewrites the entry. What `Playlist` walks to drop
     * the followers of an element being removed.
     */
    static readCcItemRefList(json: PlaylistItemType): PlaylistCcItemType[] {
        const ccItemRefList: PlaylistCcItemType[] = [];
     ...
     * turns the WHOLE entry into an error row (`Playlist.getItems`), and a
    ...
    static validate(json: AnyObjectType) {
        if (json.type === PLAYLIST_ACTION_TYPE) {
            ...
            const action = findPlaylistAction(json.data);
            if (action === null) {
            ...
            if (
                ...
                !(
                    ...
                    checkIsValidPlaylistActionTime(json.actionTime)
                ) &&
                ...
                loggerHelpers.appError(json);
                throw new Error('Invalid playlist action number');
            }
            return;
        }
        if (
