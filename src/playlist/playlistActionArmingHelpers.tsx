...
function ActionArmingInputComp({
    ...
    onChange: (arming: PlaylistActionArmingType) => void;
}>) {
...
function ActionKeyInputComp({
    ...
    const handleKeyingDown = useCallback(
        (event: ReactKeyboardEvent<HTMLInputElement>) => {
            ...
            const answer = readPlaylistActionKeyFromEvent(event);
 ...
 * with no key.
 */
async function askForPlaylistActionKey(
    runAction: PlaylistRunActionType,
    currentArming: PlaylistActionArmingType,
): Promise<PlaylistActionArmingType | null> {
    const currentKey = currentArming.actionKey;
    const defaultKey = checkIsValidPlaylistActionKey(currentKey)
        ? currentKey
        : null;
    let arming: PlaylistActionArmingType =
    ...
    if (!checkIsValidPlaylistActionKey(arming.actionKey)) {
 ...
 * on the default.
 */
export async function askForPlaylistActionArming(
    runAction: PlaylistRunActionType,
    currentArming: PlaylistActionArmingType = {},
): Promise<PlaylistActionArmingType | null> {
    ...
    if (runAction.canBeKeyArmed) {
        return await askForPlaylistActionKey(runAction, currentArming);
    }
    ...
    const isTimeDefault =
        canBeTimeArmed && checkIsValidPlaylistActionTime(currentTime);
    ...
    let arming: PlaylistActionArmingType = isTimeDefault
    ...
    if (actionTime !== undefined) {
        if (!checkIsValidPlaylistActionTime(actionTime)) {
            showSimpleToast(
 ...
 */
export async function askForPlaylistActionScreenIds(
    screenAction: PlaylistScreenActionType,
