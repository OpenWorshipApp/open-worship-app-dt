...
describe('a run action armed with a time of day', () => {
    test('only the shape this app writes is read back', () => {
        expect(checkIsValidPlaylistActionTime('07:05')).toBe(true);
        expect(checkIsValidPlaylistActionTime('00:00')).toBe(true);
        expect(checkIsValidPlaylistActionTime('23:59')).toBe(true);
        // A hand-edited file must not arm a clock on nonsense.
        expect(checkIsValidPlaylistActionTime('7:05')).toBe(false);
        expect(checkIsValidPlaylistActionTime('24:00')).toBe(false);
        expect(checkIsValidPlaylistActionTime('19:60')).toBe(false);
        expect(checkIsValidPlaylistActionTime('19:30:00')).toBe(false);
        expect(checkIsValidPlaylistActionTime(1930)).toBe(false);
        expect(checkIsValidPlaylistActionTime(undefined)).toBe(false);
    });

    test('it is READ as a 12-hour time, whatever the system says', () => {
        expect(toPlaylistActionTimeLabel('07:05')).toBe('7:05 AM');
        expect(toPlaylistActionTimeLabel('20:30')).toBe('8:30 PM');
        expect(toPlaylistActionTimeLabel('00:15')).toBe('12:15 AM');
        expect(toPlaylistActionTimeLabel('12:00')).toBe('12:00 PM');
        expect(toPlaylistActionTime(new Date(2026, 7, 5, 8, 7))).toBe('08:07');
    });

    test('a time still to come is TODAY; one gone by is nothing', () => {
        expect(toPlaylistActionDueTime('14:31', NOW)).toBe(
        ...
        expect(toPlaylistActionDueTime('07:05', NOW)).toBe(null);
        // The minute it is already in counts as gone: the operator asked for
        // 14:30, and 14:30 is behind them.
        expect(toPlaylistActionDueTime('14:30', NOW)).toBe(null);
        expect(toPlaylistActionDueTime('nonsense', NOW)).toBe(null);
    });

    test('the pill stays readable however long the wait is', () => {
        expect(toPlaylistAutoNextCountdownLabel(9)).toBe('9');
        expect(toPlaylistAutoNextCountdownLabel(59)).toBe('59');
        expect(toPlaylistAutoNextCountdownLabel(60)).toBe('1:00');
        expect(toPlaylistAutoNextCountdownLabel(605)).toBe('10:05');
        expect(toPlaylistAutoNextCountdownLabel(3600)).toBe('1:00:00');
        expect(toPlaylistAutoNextCountdownLabel(19_845)).toBe('5:30:45');
    });
});
...
describe('counting down to a time of day', () => {
    ...
    test('it needs an open run, exactly as the other clocks do', () => {
        expect(startPlaylistAutoNextAtTime(PLAYLIST_FILE_PATH, '14:35')).toBe(
    ...
    test('a time already gone by is SAID, not armed', () => {
        ...
        expect(startPlaylistAutoNextAtTime(PLAYLIST_FILE_PATH, '08:30')).toBe(
            'The set time is already due',
        );
        expect(getPlaylistAutoNextState()).toBe(null);
        unregister();
    });
    ...
    test('it steps the run when the time comes', () => {
        ...
        startPlaylistAutoNextAtTime(PLAYLIST_FILE_PATH, '14:32');
    ...
    test('the remainder is re-read from the CLOCK, not counted down', () => {
        ...
        startPlaylistAutoNextAtTime(PLAYLIST_FILE_PATH, '14:40');
        expect(getPlaylistAutoNextState()?.remainingSeconds).toBe(600);
        ...
        vi.advanceTimersByTime(1_000);
        expect(getPlaylistAutoNextState()?.remainingSeconds).toBe(59);
        expect(stepForward).not.toHaveBeenCalled();
        unregister();
    });
});
...
describe('the entry that carries a time of day', () => {
    test('a TIMEOUT is validly armed with a time and nothing else', () => {
        const playlistItem = PlaylistItem.fromJson(PLAYLIST_FILE_PATH, {
            type: PLAYLIST_ACTION_TYPE,
            data: 'next-timeout',
            actionTime: '19:30',
        });
        expect(playlistItem.actionTime).toBe('19:30');
        // Read off the row as a time, which is the whole reason to arm one.
        expect(playlistItem.title).toContain('7:30 PM');
        expect(playlistItem.actionArming).toEqual({ actionTime: '19:30' });
    });
    ...
    test('an INTERVAL is not, and its stored time is ignored', () => {
        ...
        expect(playlistItem.actionTime).toBe(null);
        expect(playlistItem.title).toContain('(8)');
    });
    ...
    test('a CC of one is armed by the ELEMENT it points at', () => {
        ...
        const elementJson = PlaylistItem.fromActionId('next-timeout', {
            actionTime: '07:05',
        });
        expect(PlaylistItem.resolveCcItemJson(elementJson)).toEqual({
            ...elementJson,
            type: PLAYLIST_ACTION_TYPE,
            data: 'next-timeout',
            actionTime: '07:05',
