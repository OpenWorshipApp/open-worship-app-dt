...
export default function PlaylistRowComp({
    ...
    // run sheet parked it (this playlist's own flag, or the document element
    // above it being parked) rather than the document itself hiding the slide.
    // Only the first kind is undone from a playlist, so the two must not look
    ...
    const isParkedByPlaylist = isDisabled && isPlaylistDisabled;
    return (
        <div
            className={
                'app-playlist-row d-flex align-items-center' +
                (isDisabled ? ' app-playlist-row-disabled' : '') +
                ...
                (isParkedByPlaylist
                    ? ' app-playlist-row-disabled-playlist'
                    : '') +
                ` ${extraClassName}`
            }
            style={{
            ...
            {...(itemUuid === undefined || itemUuid === null
                ? {}
                : { [PLAYLIST_ITEM_UUID_ATTR]: itemUuid })}
            {...(isCcRow ? { [PLAYLIST_CC_ROW_ATTR]: '' } : {})}
        >
            {isExpandable ? (
                <i
                    className={
            ...
            <span
                className={
                    ...
                    (isParkedByPlaylist ? ' app-playlist-row-label-parked' : '')
                }
                style={{ ...extraStyle }}
            >
                {label}
            ...
            {isDisabled ? (
                <i
                    className={
                        'bi app-playlist-row-disabled-icon' +
                        (isParkedByPlaylist
                            ? ' bi-slash-circle' +
