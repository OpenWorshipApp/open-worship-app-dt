...
import { toPlaylistActionDueTime } from './playlistActionTimeHelpers';
 ...
 *   seconds, or down to a TIME OF DAY (`startPlaylistAutoNextAtTime`), which is
 ...
 * them, and it does NOT replace it — see `borrowPlaylistAutoNextCycle`.
 *
...
export type PlaylistAutoNextModeType = 'timeout' | 'interval';

export type PlaylistAutoNextStateType = {
    mode: PlaylistAutoNextModeType;
     ...
     * BORROWED cycle and nothing else (`borrowPlaylistAutoNextCycle`), so it is
 ...
 */
export type PlaylistRunControllerType = {
    /**
...
const controllerState: {
    ...
    controller: PlaylistRunControllerType | null;
} = { filePath: null, controller: null };

const listeners = new Set<() => void>();
let state: PlaylistAutoNextStateType | null = null;
 ...
 */
export function usePlaylistAutoNext() {
    return useSyncExternalStore(subscribe, getSnapshot, getSnapshot);
}

export function getPlaylistAutoNextState() {
    return state;
}
 ...
 *
 * Called by the preview's cursor itself (`playlistPreviewFloatingHelpers`), so
 ...
 * number back.
 */
export function notifyPlaylistRunSelectionChanged() {
    const current = state;
    if (current === null) {
        return;
    }
 ...
 * playlist. Unregistering ENDS whatever is armed: the widget closing (or being
 * pointed at another playlist) is the run being over, and a timer that outlived
 * it would step a list nothing is looking at.
 */
export function registerPlaylistRunController(
    filePath: string,
    controller: PlaylistRunControllerType,
) {
...
/** The open run on THIS playlist, or null — what every trigger goes through. */
function findPlaylistRunController(filePath: string) {
    return controllerState.filePath === filePath
        ? controllerState.controller
        : null;
}

/** Whether a run is open on this playlist at all — what a trigger needs. */
export function checkPlaylistAutoNextIsRunnable(filePath: string) {
    return findPlaylistRunController(filePath) !== null;
}
...
export function stopPlaylistAutoNext() {
    if (tickingId !== null) {
        clearInterval(tickingId);
        tickingId = null;
    }
 ...
 */
function borrowPlaylistAutoNextCycle(
    mode: PlaylistAutoNextModeType,
    seconds: number,
    dueTime: number | null,
) {
 ...
 * nothing at all.
 */
export function setPlaylistAutoNextPaused(isPaused: boolean) {
    const current = state;
 ...
 * `borrowPlaylistAutoNextCycle`.
 *
 * Refuses when no run is open on THIS playlist — a run action clicked in the tree
 * with the preview closed (or open on another sheet) must say so rather than
 * quietly stepping a list nobody is looking at.
 */
export function startPlaylistAutoNext(
    filePath: string,
    mode: PlaylistAutoNextModeType,
    seconds: number,
    ...
    if (
        !checkPlaylistAutoNextIsRunnable(filePath) ||
    ...
    if (borrowPlaylistAutoNextCycle(mode, seconds, dueTime)) {
        return true;
    }
    stopPlaylistAutoNext();
    state = {
 ...
 * loud rather than rolled over to tomorrow — see `toPlaylistActionDueTime`.
 *
 ...
 */
export function startPlaylistAutoNextAtTime(filePath: string, time: string) {
    if (!checkPlaylistAutoNextIsRunnable(filePath)) {
        return 'Open the playlist preview to use this action';
    }
    const dueTime = toPlaylistActionDueTime(time, Date.now());
    if (dueTime === null) {
        return 'The set time is already due';
    }
    const seconds = Math.ceil((dueTime - Date.now()) / 1000);
    return startPlaylistAutoNext(filePath, 'timeout', seconds, dueTime)
        ? null
 ...
 */
export function toPlaylistAutoNextCountdownLabel(remainingSeconds: number) {
 ...
 *
 * Lives here rather than in `playlistHelpers` because both of those callers must
 * reach it and `playlistCcApplyHelpers` may not import back into
 * `playlistHelpers` — this module is under both of them and takes `PlaylistItem`
 * as a TYPE only, so nothing is added to the cycle either way.
 *
 * A run open on ANOTHER playlist (or none at all) is told, not swallowed: an
 * operator whose countdown did not start has to know it did not start.
 */
export function firePlaylistRunAction(playlistItem: PlaylistItem) {
    const runAction = playlistItem.runAction;
    if (runAction === null) {
        return;
    }
    const message = runAction.start(playlistItem);
    if (message !== null) {
 ...
 */
export function jumpPlaylistRunToCcItem(
    playlistFilePath: string,
    ccItem: PlaylistItem,
) {
    const controller = findPlaylistRunController(playlistFilePath);
    if (controller === null) {
        return 'Open the playlist preview to use this action';
    }
    const { uuid } = ccItem;
    if (uuid === null || !controller.jumpToUuid(uuid)) {
        return 'The element to jump to is not in this playlist';
    }
    return null;
}
