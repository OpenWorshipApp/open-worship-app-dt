 ...
 * seconds again — and `PlaylistItem.actionTime` wins, so the stale one would be
 * the one that stopped mattering without ever being removed.
 */
function applyPlaylistActionArming(
    itemJson: PlaylistItemType,
    { actionNumber, actionTime, actionKey }: PlaylistActionArmingType,
) {
 ...
 * its own shortcut.
 */
function checkPlaylistActionKeyIsFree(
    itemJsonList: PlaylistItemType[],
    ...
    const isTaken = itemJsonList.some((itemJson, index) => {
        return (
            index !== skipIndex &&
            itemJson.type === PLAYLIST_ACTION_TYPE &&
            itemJson.actionKey === actionKey
        );
    });
    if (!isTaken) {
    ...
    showSimpleToast(
        tran('Keyboard Event'),
        `${tran('This shortcut is already used in this playlist')}: ${actionKey}`,
    );
    return false;
}
 ...
 */
function ensurePlaylistItemUuid(itemJson: PlaylistItemType) {
    if (!checkIsValidPlaylistItemUuid(itemJson.uuid)) {
        itemJson.uuid = genPlaylistItemUuid();
    }
 ...
 */
function dropPlaylistCcItemsOfUuid(
    itemJsonList: PlaylistItemType[],
    uuid: string | null,
) {
    if (uuid === null) {
    ...
    for (const itemJson of itemJsonList) {
        const ccItemRefList = PlaylistItem.readCcItemRefList(itemJson);
        if (
...
export default class Playlist extends AppEditableDocumentSourceAbs<PlaylistType> {
    ...
    async getItems() {
        ...
        // CC's uuid means — see `PlaylistItem.bindCcSources`.
     ...
     *
     * Serialized per playlist file, because this is a read-modify-write and it
    ...
    private async updateItemJsonList(
        ...
        return await unlocking(`playlist-items-${this.filePath}`, async () => {
    ...
    /**
     * An action is not dragged from anywhere — it is chosen from the playlist's
    ...
    async addActionItem(
        ...
        try {
            return await this.insertItemJson(
                PlaylistItem.fromActionId(actionId, arming, screenIds),
                toIndex,
            );
        } catch (error: any) {
            handleError(error);
    ...
    async removeItemAtIndex(index: number) {
        return await this.updateItemJsonList((itemJsonList) => {
            ...
            dropPlaylistCcItemsOfUuid(itemJsonList, itemJson.uuid ?? null);
        });
    }
    ...
    async duplicateItemAtIndex(index: number) {
        return await this.updateItemJsonList((itemJsonList) => {
            ...
            const newItemJson = cloneJson(itemJson) as PlaylistItemType;
            newItemJson.uuid = genPlaylistItemUuid();
    ...
    private async updateItemJson(
        index: number,
        mutate: (itemJson: PlaylistItemType) => void,
    ) {
        return await this.updateItemJsonList((itemJsonList) => {
     ...
     */
    async setItemActionArming(index: number, arming: PlaylistActionArmingType) {
        return await this.updateItemJsonList((itemJsonList) => {
            if (itemJsonList[index] === undefined) {
                return false;
            }
            ...
            const itemJson = { ...itemJsonList[index] };
            applyPlaylistActionArming(itemJson, arming);
            itemJsonList[index] = itemJson;
        });
    }
    ...
    private async updateItemCcList(
        ...
        slideId: number | null,
        mutate: (
            ccItemRefList: PlaylistCcItemType[],
            hostItemJson: PlaylistItemType,
            itemJsonList: PlaylistItemType[],
        ) => boolean | void,
    ) {
        return await this.updateItemJsonList((itemJsonList) => {
            ...
            (ccItemRefList, hostItemJson, itemJsonList) => {
                ...
                if (slideId === null) {
                    const maxCcItemCount =
                        PlaylistItem.getMaxCcItemCount(hostItemJson);
                ...
                const sourceJson = resolveSourceJson(
                    itemJsonList,
                    slideId === null &&
                        PlaylistItem.checkIsCcTargetHost(hostItemJson),
                );
                if (sourceJson === null) {
                    showSimpleToast(
                        tran('Adding CC Element'),
                ...
                const uuid = ensurePlaylistItemUuid(sourceJson);
    ...
    private async updateItemCcItemRef(
        ...
        slideId: number | null,
        ccIndex: number,
        mutate: (ccItemRef: PlaylistCcItemType) => void,
    ) {
        return await this.updateItemCcList(index, slideId, (ccItemRefList) => {
            if (ccItemRefList[ccIndex] === undefined) {
                return false;
     ...
     * exactly what dropping the same payload on the playlist itself would have
    ...
    /**
     * Attach an element that is ALREADY LISTED — in this playlist or another —
     ...
     * playlist sidesteps that entirely and works for every kind at once.
     *
     ...
     * playlist — and it would be a follower naming a line this run sheet does not
     ...
     * trusted: a playlist can be edited between the drag starting and the drop.
     */
    async addItemCcFromItemIndex(
        ...
        try {
            const sourceItems =
                await Playlist.getInstance(sourceFilePath).getItems();
            ...
            return await this.attachItemCcItem(
                ...
                (itemJsonList, isTarget) => {
                    if (
                        sourceJson === null ||
                        PlaylistItem.resolveCcItemJson(sourceJson, isTarget) ===
                            null
                    ) {
                        return null;
                    }
