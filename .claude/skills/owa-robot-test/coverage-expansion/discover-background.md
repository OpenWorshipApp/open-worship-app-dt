...
| PM-47 | Header **Add-items `+`** button (`RenderPathTitleComp`) | 🖱️ | — | any bg tab, dir set | click the `bi-plus-lg` (title "Add items") | opens a context menu: `Add Local Files` (→OS file dialog, EX-01) plus per-tab download items (`Download From URL`, `Paste Image` for images, `Add URL`/`New File` for web), all at one level and each listed once | `RenderPathTitleComp.tsx:36-45`, `FileListHandlerComp.tsx:148-162`, `droppingFileHelpers.ts:170-180` | GAP — user-flagged header add button |
