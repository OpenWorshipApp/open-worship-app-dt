...
| `…Left → PlaylistListComp` (Playlists list) | [playlist/PlaylistListComp.tsx](../../../../src/playlist/PlaylistListComp.tsx) | **Every build** — the `isDev` gate was removed in `203d35cc` and this widget took the slot the old `LyricListComp` held (lyrics are now rows of the Documents list). 🖱️ a card header → open/close; ⇕ drop a background / document / slide / bible item / foreground / audio to add; 🖱️ an element row → present it (a document row opens its previewer); ⇕ a row onto another row of the SAME playlist → reorder; ⇕ a row onto another row **while it shows `app-playlist-row-dragging-over-cc`** → attach as a **CC element** instead of reordering; 🖱️R an element → Reveal Original / Show on Screens / **Set Specific Screen** / **Add CC Elements** / Move up / Move to Top / Move down / Move to Bottom / **Duplicate** / Choose Color / **Disable** / Remove; 🖱️R the playlist row itself → **Add Action** (three-level menu: 5 clears · **Other Clear FG Items** ▸ 8 per-widget clears · 4 run actions) / Export / Import; header `bi-window-stack` → floating run-sheet **player** (arrow/Space step it, a `Keyboard Event` line answers its own shortcut). Sub-components: `PlaylistFileComp`, `PlaylistItemComp`, `PlaylistRowComp`, `PlaylistCcRowsComp`, `PlaylistScreenPinComp`, `PlaylistDocumentSlidesComp`, `PlaylistPreviewFloatingComp`, `PlaylistItemPreviewComp`; logic in `playlistActionHelpers` / `playlistCcHelpers` / `playlistAutoNextHelpers` / `playlistArchiveHelpers` / `playlistPreviewFloatingHelpers`. See knowledge-base §14, test-plan §S20, matrix PL-10 / PL-29 / PL-32..PL-76 / PL-81..PL-100 (**playlist deep mode**). |
