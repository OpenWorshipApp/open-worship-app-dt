...
follow-up under *How do I add a song?* was found guessing a menu item the
...
- **The follow-up in the same tab is where the manual's gaps show.** Every
  first question in the corpus has a recipe; *and how do I undo that?* had
...
- **A question about the WINDOW's own state cannot be answered from the
  manual.** *How much has this chat cost?* opened the recipe and a free model
...
- **The driver's provider pick rewrites the window's default.** Choosing
  *Free* in the head row to grade the free tier stored *Free* as what every
