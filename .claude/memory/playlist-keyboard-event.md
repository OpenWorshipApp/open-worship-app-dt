...
([[playlist-screen-actions]]), pink, badge `⌨`.
...
- **Armed with a SHORTCUT, a third alternative to the two clocks.**
  `PlaylistActionArmingType` now carries `actionKey` beside `actionNumber`/`actionTime`, and
  `applyPlaylistActionArming` deletes all three before writing one — same rule, one more field.
  Registry flag: `canBeKeyArmed` (only this action; its `number` is null, so the clock half of
  `askForPlaylistActionArming` is skipped entirely by an early branch).
- **Ctrl and Shift ONLY, and at least one of them.** `playlistActionKeyHelpers.ts` is a LEAF
...
- **UNIQUE per playlist**, enforced in `Playlist` (the write funnel), not at the question:
  `insertItemJson` + `setItemActionArming` both call `checkPlaylistActionKeyIsFree`, and
  ...
  introduced to end. `collectPlaylistRunShortcutKeys` still dedupes on read (first wins) — a
...
- **It is the ONE run action that resolves screens.** Its CCs are ordinary FOLLOWERS and it
  ...
  dead. `sendPlaylistItemToScreens` branches on `PlaylistItem.hostsCcFollowers`
  ...
  With NOTHING attached it falls back to `firePlaylistRunAction` for the toast.
- **`checkIsCcTargetHost` is now per-action (`ccItemsAreTargets`), not "is it a run action".**
  ...
  `0 | 1` to `number` (Infinity here). See [[playlist-cc-elements]].
- **`isScreenPinnable` includes it** — the pin is read for the followers' screens, exactly as
  a document's pin is read for its slides.
- **One `PlaylistRunShortcutComp` per shortcut, keyed by it.** `useKeyboardRegistering`
...
- **The run now stops on EVERYTHING that is not parked**, and unfolds what it lands on —
  `findNextPlaylistPreviewIndex` lost its `checkIsEnterable` gate and `landPlaylistRunOnIndex`
  calls `expandPlaylistPreviewItem`. See [[playlist-preview-run-player]].
- The `Add Action` menu is now THREE levels: the eight per-widget FG clears fold behind
  `Other Clear FG Items`. `playlistActionMenuList` (shape) is separate from `playlistActionList`
  (flat registry, what an id resolves against) — only the component reads the former.
- Unit tests: `src/playlist/playlistActionKey.test.ts`.
