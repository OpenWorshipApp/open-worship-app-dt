...
description: Playlist CC elements ride along with their host's present; they are UUID references to a listed line, and they never choose a screen themselves
metadata:
  type: project
---
...
- **A CC stores `{uuid, screenIds?}` and NOTHING else.** Every entry carries a `uuid`
  (`genPlaylistItemUuid`, minted on add; a DUPLICATE is re-keyed; a legacy entry gets one
  ...
  (`PlaylistItem.resolveCcItemJson`), so **re-arming a `Next: Timeout` element re-arms
  every CC of it at once**. Its own pin is the one override; `Change Seconds` is gone from
  the CC row menu.
- **Resolution needs the LIST, not the item.** `Playlist.getItems` calls
  `PlaylistItem.bindCcSources(items)`, which hands every entry one shared uuid→item map
  (built only when some entry has CCs). A `PlaylistItem` built alone — a test, a CC —
  resolves nothing, on purpose. Rows are stamped `data-playlist-item-uuid` (bare uuid,
  unscoped: it is globally unique), which is what `notifyPlaylistCcOrigin` and the run's
  `jumpToUuid` query.
- **Every CC has an element of its own in the sheet.** A payload dropped from outside is
  ...
  playlist is copied in the same way (a uuid is an identity within one file). Both happen
  in ONE write (`Playlist.attachItemCcItem`), so a refused attach never strands an
  appended line. Removing an element DROPS every CC of it
  (`dropPlaylistCcItemsOfUuid`), and a reference answering to nothing is simply not
  drawn.
...
- **One follower does not go to a screen at all**: a `Next: Timeout` (a RUN action, see
  ...
  BORROWS its cycle rather than replacing it; see [[playlist-auto-next]].
- **A `Jump to` host turns the CC into a POINTER, not a follower** — it names the line the
  run goes to. `PlaylistItem.checkIsCcTargetHost` switches `resolveCcItemJson(json, isTarget)`
...
- The `.owapl` archive no longer walks CC jsons for paths: a CC holds none, and what it
  points at is a listed entry walked in its own right ([[playlist-archive-owapl]]).
- **Migrating a real playlist means migrating `<file>.owp.histories/<n>-head`**, not just
  the `.owp` — `AppEditableDocumentSourceAbs.getJsonData` reads the editing-history HEAD,
...
- The bug that forced the rebuild: a derived key could not tell `Next: Timeout (3)` from
  ...
  uuid has nothing to disambiguate. `toPlaylistCcOriginKey` and every `*OriginKey` helper
