---
name: playlist-panel-no-longer-dev-only
description: The Playlists panel lost its isDev gate in commit 203d35cc and took the removed Lyric List's slot — every doc saying "dev builds only" is stale
metadata:
  type: project
---
...
two widgets — **Documents** (`v1`) and **Playlists** (`v2`) — because the separate
**Lyric List** widget was removed at the same time (see [[lyric-in-documents-list]]).
The Playlists panel therefore ships in packaged builds.
...
**How to apply:** never mark a PL playlist row BLOCKED "because it's dev-only", and don't
...
See also [[playlist-references-vs-presets]] for what the panel stores.
