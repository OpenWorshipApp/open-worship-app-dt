---
name: playlist-screen-show-hide
description: Screen: Show / Screen: Hide are the only playlist actions that NAME their screens — asked at add time, stored in the ordinary screenIds pin, and no ambient fallback ever
metadata:
  type: project
---

`screen-show` / `screen-hide` (added 2026-08-06, branch refactor24) are SCREEN actions
(`target: 'screen'`, see [[playlist-screen-actions]]) whose `apply` writes
...
walked by a `Next: Interval`, darken it at the end — see [[playlist-auto-next]]).
...
- **They are the one family with `requiresScreenIds: true`**, and that flag is the whole
  design. `PlaylistFileComp`'s Add Action menu calls `askForPlaylistActionScreenIds`
...
- **No ambient fallback, ever.** `sendPlaylistItemToScreens` refuses with the same toast
  ...
  screen that never goes up. `applyPlaylistCcItemsOnScreenIds` has its own branch for the
...
- **`apply` reads `isShowing` before writing.** The setter does real show/hide window work
  ...
  must not pay for it.
- **`ccItemCount` moved to `PlaylistActionBaseType`** so a SCREEN action can refuse hosts
  too (these two are `0`, every clear stays `Infinity`); `PlaylistItem.getMaxCcItemCount`
...
- Matrix row `PL-100`; the playlist deep mode scope is `PL-81..100` (67 rows) now.
