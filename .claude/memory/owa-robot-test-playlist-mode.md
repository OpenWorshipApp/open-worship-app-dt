---
name: owa-robot-test-playlist-mode
description: `/owa-robot-test playlist` is a tracked 11-phase MODE over 66 run-sheet rows, not a focus area that trims the run
metadata:
  type: project
---

Asking the skill for `playlist` (or `run sheet` / `.owp`) selects **playlist deep mode** —
...
coverage accounting ON (`"focus": "playlist"` in `coverage-<runid>.json`, resumable) and
runs P0..P10 in order, so a playlist run is auditable the same way a full-coverage run is.
...
- **Fixture discipline:** build `zz-robot-<runid>` and tear it down. The panel has no save
  button, so every gesture writes the user's real `.owp` immediately.
...
- KB §14 was also corrected in the same pass: its run-player rules still said audio,
  ...
  the only reason ([[playlist-preview-run-player]]). New §14.9..§14.15 cover the action
