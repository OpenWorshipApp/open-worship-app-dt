...
- `target: 'screen'` → `apply(screenManager)`. Fifteen ship: the five that mirror the
  ...
  what is on it — see [[playlist-screen-show-hide]].
- `target: 'run'` → `start(playlistItem)` and drives the RUN rather than a screen (added
  2026-08-05, branch refactor24). Four ship: `next-interval` and `next-timeout`, which drive
  [[playlist-auto-next]]; `jump-to`, the sheet's GOTO; and `keyboard-event`, the hotkey — the
  ONE of them that reaches a screen at all, and only through its CC elements
  (see [[playlist-keyboard-event]]).

**The `Add Action` menu is three levels now**: the eight per-widget FG clears fold behind one
`Other Clear FG Items` row. `playlistActionMenuList` is the menu's SHAPE (a flat action or a
`{label, iconName, color, actionList}` group, told apart by `checkIsPlaylistActionGroup`);
`playlistActionList` stays the flat registry an id resolves against. Only `PlaylistFileComp`
