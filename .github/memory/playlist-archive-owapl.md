---
name: playlist-archive-owapl
description: .owapl.tar.gz bundles a playlist with the WHOLE documents behind its slide references; import resolves every destination folder up front so a failure writes nothing
metadata:
  type: project
---

`src/playlist/playlistArchiveHelpers.ts` — export/import of a playlist as
`<name>.owapl.tar.gz` via `tarCreate`/`tarExtract` (**not** a zip: the app has no zip
...
playlist-specific parts. Fix a rule in the shared module, not here.

Layout: `manifest.json` + `playlist.json` + `files/`. Because slides are stored as
references ([[playlist-references-vs-presets]]), the bundle carries the **whole document**
...
- **`resolveKindDirPaths` runs BEFORE any file is copied.** `getKindDirPath` throws when a
  ...
  and no playlist to show for it. Same for the playlists folder itself.
- Archive paths are validated (`..` or a backslash → `Invalid playlist archive file path`).
- **Reuse is decided by MD5, not by name.** A same-named file in the destination folder is
  ...
  as different. An existing `.bg.json` is never clobbered; the playlist file itself
  de-duplicates the same way, so importing twice is safe but does add a playlist.
