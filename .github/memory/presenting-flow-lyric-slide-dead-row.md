---
name: presenting-flow-lyric-slide-dead-row
description: A lyric ATTACHMENT slide in a presenting flow could not be read back — getSlideById only searched getSlidesQuick(), which never lists them (fixed 2026-08-06)
metadata:
  type: project
---
...
`null` on a miss. A presenting flow stores a lyric slide by id, so an attachment slide dragged into
...
slow path only runs for them), and an unresolvable presenting flow reference now toasts
`Showing PresentingFlow Item / Fail to read file data` instead of failing silently.
...
[[lyric-subsystem-architecture]], [[owa-robot-test-presenting-flow-mode]].
