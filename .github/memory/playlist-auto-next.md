---
name: playlist-auto-next
description: The playlist run can walk itself (one clock, two endings) and jump; the run's SELECTION CHANGING cancels a timeout and restarts an interval — raw clicks/keys mean nothing
metadata:
  type: project
---

`src/playlist/playlistAutoNextHelpers.ts` (added 2026-08-05, branch refactor24) is the
run sheet's own clock, armed by the two `next-*` run actions of
[[playlist-screen-actions]]. ONE slot for the whole app, in memory only, ticking once a
...
(`notifyPlaylistRunSelectionChanged`, called from `notifySelecting` in
`playlistPreviewFloatingHelpers`, which is the only place that knows the run moved):
...
- **timeout** — counts down once, forces the next selection, spent. The run going
  ...
  **time of day** instead of a count of seconds (`canBeTimeArmed`, stored as `actionTime`
  = `HH:mm`, `startPlaylistAutoNextAtTime`): a service starts at 7:05, and counting the
...
that NAMES the line to go to (`ccItemCount: 1`, matched by `ccOriginKey`), never armed with
a number, and never a CC itself. `PlaylistItem.checkIsCcTargetHost` is what makes its CC a
POINTER rather than a follower, so `toCcItemJson(json, isTarget)` lets it name a DOCUMENT —
the usual target, and the one thing an ordinary CC can never be.

**A jump IS the next-key aimed at a chosen line** — same landing (`landPlaylistRunOnIndex`),
...
itself. See [[playlist-cc-elements]].

**How to apply:**

- Every path that ARMS one settles the cursor FIRST (the preview frame's `onClickCapture`
  runs before the button's `onClick`; `landPlaylistRunOnIndex` selects before it fires), so
  arming is never cancelled by the very gesture that armed it. Keep it that way.
- A REPAIR is not a move: `resolvePlaylistPreviewSelectedIndex` writing back the position a
...
- The run CONTROLLER (`stepForward` + `jumpToOriginKey`) is REGISTERED by
  `PlaylistPreviewFloatingComp` (with its filePath), never implemented here — nothing in this module may hold a run sheet or a document's slides.
  Unregistering stops the clock. Arming from anywhere with no matching open run returns
  false, and `sendPlaylistItemToScreens` toasts.
...
- A CC'd timeout is fired from `applyPlaylistCcItemsOnScreenIds` — i.e. only once the
...
- `firePlaylistRunAction` lives in THIS module, not `playlistHelpers`, so that
  `playlistCcApplyHelpers` can call it without importing back into `playlistHelpers`
  (which imports it). `PlaylistItem` is a TYPE-only import here for the same reason.
- `start(playlistItem)` answers with a `tran` KEY to toast (or null), so each action says
...
- `actionNumber` and `actionTime` are ALTERNATIVES, so both the question and the setter
  deal in one `PlaylistActionArmingType` and `applyPlaylistActionArming` DELETES the other
  — `actionTime` wins in `PlaylistItem`, so a stale number left behind would be the field
  that quietly stopped mattering. `PlaylistItem.actionTime` also answers null for an action
  that may not carry one, so a hand-edited interval is ignored in the row, the question and
  the clock at once.
- Unit tests: `src/playlist/playlistAutoNext.test.ts` (fake timers),
  `playlistActionTime.test.ts` (the clock-time half, `vi.setSystemTime`) +
  `playlistCcPropagation.test.ts` (the CC half).
