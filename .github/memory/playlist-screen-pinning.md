---
name: playlist-screen-pinning
description: "A playlist entry (or one slide of a document) can be PINNED to screens; the preset rides chooseScreenIds, and isForceChoosing deliberately outranks it"
metadata: 
  node_type: memory
  type: project
...
- **`sendPlaylistItemToScreens` is NOT the only present path**, despite its comment. The
  ...
  [[playlist-cc-elements]] every one of those paths ALSO has to arm the CC followers, so
...
- **Slide thumbnails get theirs by CONTEXT** (`PresetScreenIdsContext` in
  ...
  playlists. Default `[]` = today's behaviour everywhere else.
- **`screenChoosingHelpers.ts` must stay a leaf.** `ScreenEventHandler` imports it, so a
  ...
  picker lives in `PlaylistScreenPinComp.tsx` instead — the same place `chooseColorNote`
...
`src/playlist/playlistScreenIds.test.ts`. See [[playlist-references-vs-presets]],
[[playlist-preview-run-player]], [[playlist-screen-actions]].
