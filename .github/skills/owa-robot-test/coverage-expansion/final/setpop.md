...
| ST-22 | "Other General Options" card holds ONLY `Clear All Settings` | look at the card | **MOVED 2026-08-09**: `Reset Widgets Size` is a native **View** menu entry now (NAV-21), so the card must show one button and no leading `<hr>` (src: src/setting/SettingGeneralOtherOptionsComp.tsx) |
