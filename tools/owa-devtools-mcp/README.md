...
- **https only, public addresses only.** `127.0.0.1` is where the app serves
  its own CDP and MCP doors, and neither has a credential (`MC-01`), so a
...
matcher now asks whether a control is *painted*:
...
- Anything with no box at all — `display: none`, a closed panel — is
  still "not on screen", and says so instead of being revealed.
