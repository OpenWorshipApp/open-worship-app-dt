...
describe('selectGuideStepsForTopic', () => {
  const steps = [
    { text: 'Type a full reference such as John 3:16.' },
    {
      text: 'If the words are too small, reveal Font Size and make them larger.',
    },
    { text: 'Read two Bible versions next to each other.' },
    { text: 'If you lost a passage, click a recent reference row.' },
  ];

  it('starts a broad recipe at the one task the person asked for', () => {
    expect(selectGuideStepsForTopic(steps, 'The words are too small.')).toEqual(
      [steps[1]],
    );
    expect(selectGuideStepsForTopic(steps, 'I lost the passage.')).toEqual([
      steps[3],
    ]);
    expect(
      selectGuideStepsForTopic(steps, 'Put two versions side by side.'),
    ).toEqual([steps[2]]);
  });

  it('keeps the whole recipe when the topic is too vague to choose safely', () => {
    expect(selectGuideStepsForTopic(steps, 'Please help me.')).toEqual(steps);
  });
});
...
describe('toGuideSteps', () => {
  it('keeps a bold control label and drops keystrokes and stressed prose', () => {
    ...
    expect(steps.map((step) => step.finds)).toEqual([['Bible Reader'], [], []]);
  });
...
describe('dropStepsAlreadyDone', () => {
  it('drops a step that only says to look at the app window', () => {
    ...
    expect(dropStepsAlreadyDone(steps, '/reader.html')).toEqual([steps[1]]);
  });
  ...
  it('reads only the first sentence for where a step is going', () => {
    ...
    expect(
      dropStepsAlreadyDone(
        [{ text: 'Click the Presenter tab. Then wait.', finds: [] }, steps[1]],
        '/presenter.html',
      ),
    ).toHaveLength(1);
  });
  ...
  it('knows the Document Editor by the words on its tab', () => {
    ...
    expect(dropStepsAlreadyDone(steps, '/appDocumentEditor.html')).toHaveLength(
      1,
    );
    expect(
      dropStepsAlreadyDone(
  ...
  it('leaves a step alone in a window nothing declares', () => {
    ...
    expect(dropStepsAlreadyDone(steps, '/nothing-like-it.html')).toEqual(steps);
  });
});
...
describe('detectRecipeWindow', () => {
  ...
  it('says nothing when the first step is not a go-there step', () => {
    expect(
      detectRecipeWindow([{ text: 'The Settings window shows four tabs.' }]),
    ).toBeNull();
  });
...
describe('the guide runtime', () => {
  it('can hover a surface to reveal the next control without moving the real mouse', async () => {
    document.body.innerHTML = [
      '<button id="surface">Passage controls</button>',
      '<button id="font" style="display:none">Font Size</button>',
    ].join('');
    document.getElementById('surface').addEventListener('mouseover', () => {
    ...
    startGuide({
      ...
      steps: [
        {
          text: 'Move over the passage controls.',
          find: 'Passage controls',
          action: 'hover',
        },
        { text: 'Use Font Size.', find: 'Font Size' },
      ],
    });

    const afterHover = await window.__owaGuide.act();
    expect(afterHover.lastResult).toEqual({
      done: true,
      did: 'hovered',
      label: 'Passage controls',
    });
    await new Promise((resolve) => setTimeout(resolve, 750));
    expect(window.__owaGuide.status()).toMatchObject({
      stepNumber: 2,
      find: 'Font Size',
      isTargetFound: true,
    });
  ...
  it('changes a range slider instead of clicking its nearby label', async () => {
    document.body.innerHTML = [
      '<div>Font Size</div>',
  ...
  it('makes a named range larger relative to its current value', async () => {
    document.body.innerHTML = [
      '<div>Font Size</div>',
      '<input id="size" type="range" aria-label="Font Size" min="5" max="40" step="2" value="17">',
    ].join('');
    startGuide({
      mode: 'demo',
      steps: [
    ...
    expect(document.getElementById('size').value).toBe('25');
  });

  it('chooses a named option in a picker', async () => {
    document.body.innerHTML = [
      '<select id="view" aria-label="Resources">',
      '<option value="s">Find</option>',
      '<option value="r" selected>Resources</option>',
      '</select>',
    ].join('');
    startGuide({
      mode: 'demo',
    ...
    expect(document.getElementById('view').value).toBe('s');
  });

  it('opens a three-dot auto-hide footer before changing its control', async () => {
    document.body.innerHTML = [
      '<style>',
      '.app-auto-hide > div { visibility: hidden; }',
      '.app-auto-hide.auto-hide-show > div { visibility: visible; }',
      '</style>',
      '<div id="preview">',
      '  <div id="footer" class="app-auto-hide">',
      '    <div><input id="size" type="range" aria-label="Font Size" min="5" max="40" step="2" value="17"></div>',
      '  </div>',
      '  <i id="reveal" class="auto-hide-button" title="Reveal Hidden Controls"></i>',
      '</div>',
    ].join('');
    document.getElementById('reveal').addEventListener('click', () => {
      document.getElementById('footer').classList.add('auto-hide-show');
    });
    startGuide({
      mode: 'demo',
      steps: [
    ...
    expect(host.shadowRoot.querySelector('.hint').textContent).toContain(
      'open the hidden controls first',
    );
    const opened = await window.__owaGuide.act();
    expect(opened.lastResult).toMatchObject({
      done: true,
      did: 'revealed',
      more: 'Font Size',
    });
    expect(document.getElementById('footer').classList).toContain(
      'auto-hide-show',
    );
    expect(document.getElementById('size').value).toBe('17');

    const changed = await window.__owaGuide.act();
    expect(changed.lastResult).toMatchObject({
      done: true,
      did: 'typed',
      value: '25',
    });
  ...
  it('labels and runs the only step of a one-step demo as Do it', async () => {
    document.body.innerHTML =
      '<input id="size" type="range" aria-label="Font Size" min="5" max="40" step="2" value="17">';
    startGuide({
      mode: 'demo',
    ...
    const button = document
      ...
      .shadowRoot.querySelector('.next');
    expect(button.textContent).toBe('Do it');
    button.click();
    // A quick second press while the result is still being shown is
    // ignored rather than applying the relative range change twice.
    button.click();
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(document.getElementById('size').value).toBe('25');
    expect(window.__owaGuide.status().lastResult).toMatchObject({
      done: true,
      did: 'typed',
      value: '25',
    });
  ...
  it('advances when the target appears during the pre-action wait', async () => {
    document.body.innerHTML = '';
    startGuide({
      mode: 'demo',
      steps: [
        { text: 'Choose John.', find: 'John', action: 'click' },
        { text: 'Choose chapter 3.', find: 'Chapter 3' },
      ],
    });
    setTimeout(() => {
      const button = document.createElement('button');
      button.textContent = 'John';
      button.addEventListener('click', () => button.remove());
      document.body.appendChild(button);
    }, 50);
    const acted = await window.__owaGuide.act();
    expect(acted.lastResult).toMatchObject({
      done: true,
      did: 'clicked',
      label: 'John',
    });
    expect(acted.lastResult.more).toBeUndefined();
    await new Promise((resolve) => setTimeout(resolve, 750));
    expect(window.__owaGuide.status().stepNumber).toBe(2);
  });

  it('advances when the person hovers the ringed surface themselves', () => {
    document.body.innerHTML = '<div id="surface" role="button">More</div>';
    startGuide({
      mode: 'show',
      steps: [
        { text: 'Move over More.', find: 'More', action: 'hover' },
        { text: 'Done.', finds: [] },
      ],
    });
    document
  ...
  it('beats the ring while it waits on the user, holds still to act', () => {
    ...
    const ringOf = () => {
      return document
        .getElementById('owa-guide-host')
        .shadowRoot.querySelector('.ring').dataset.waiting;
    };
    startGuide({
      mode: 'show',
      steps: [{ text: 'Click it.', finds: ['Bible Reader'] }],
...
describe('what the guide costs while it sits there', () => {
  ...
  it('installs no timer just for being asked a question', () => {
    const live = countTimers(() => {
      new Function(`return ${genGuideExpression('status()')}`)();
    });
...
describe('what a step offers as a control to ring', () => {
  ...
  it('offers each candidate once', () => {
    const [step] = toGuideSteps('1. Press **Videos** on the **Videos** tab.\n');
  ...
  it('offers the demo even though it rings nothing yet', () => {
    ...
    const guide = startGuide({
      ...
      steps: [
        {
          text:
            'Right-click an empty part of the list and choose' +
            ' Download From URL.',
  ...
  it('right-clicks the named divider and names the item it brought up', async () => {
    ...
    const guide = startGuide({
      ...
      steps: [
        {
          text:
            'Right-click the divider between Document List and' +
  ...
  const startStuck = () => {
    return startGuide({
      ...
      steps: [
        {
          text:
            'The verse renders in the preview panel.' +
    ...
    document.dispatchEvent(
      new CustomEvent('owa-guide-help-answer', {
        detail: {
          ...
          text:
            'Double-click the verse in the preview panel on the' +
...
describe('a control behind a menu or a floating panel', () => {
  ...
  it('closes a right-click menu by its backdrop and stays on the step', async () => {
    ...
    const first = await window.__owaGuide.act();
    expect(first.lastResult).toMatchObject({
      done: true,
      did: 'closed',
      closed: 'the menu',
    });
  ...
  it('closes a floating panel with its own button', async () => {
    ...
    const host = document.getElementById('owa-guide-host');
    expect(
      host.shadowRoot.querySelector('[class*=hint]').textContent,
    ).toContain('ringed ✕');
...
describe('a control that only resembles the step', () => {
  ...
  it('still presses a control named exactly, shortcut and all', async () => {
    ...
    startGuide({
      ...
      steps: [
        {
          text: 'Press Toggle showing screen.',
          finds: ['Toggle showing screen'],
        },
        { text: 'Done.', finds: [] },
      ],
    });
