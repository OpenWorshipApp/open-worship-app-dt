...
#
# Usage:
#   ./build-qjs.sh [version]
#   ./build-qjs.sh 2026-06-04   # explicit (releases are date-stamped)
#   ./build-qjs.sh              # defaults to 2026-06-04
#
# Knobs (env overrides):
...
# --- build knobs -------------------------------------------------------------
# QuickJS uses a plain Makefile (no ./configure). On Darwin the Makefile
# auto-selects clang + system `ar` (CONFIG_DARWIN -> CONFIG_CLANG,
# CONFIG_DEFAULT_AR), and LTO is off by default, so a stock `make qjs` just
...
if ((${#missing[@]})); then
    ...
    echo "Install Xcode command line tools (macOS) or build-essential (Linux)." >&2
    exit 1
fi
...
# --- extract -----------------------------------------------------------------
# macOS `tar` (bsdtar) decodes .tar.xz natively; GNU tar shells out to `xz`.
if [[ ! -d "$src_dir" ]]; then
...
# (bash 3.2 on macOS errors on "${empty[@]}" otherwise).
