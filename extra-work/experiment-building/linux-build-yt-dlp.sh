...
#
# Usage:
#   ./linux-build-yt-dlp.sh [version]
#   ./linux-build-yt-dlp.sh                 # defaults to 2026.07.04 (shipped tag)
#   ./linux-build-yt-dlp.sh 2026.07.04      # a pinned release tag (reproducible)
#   ./linux-build-yt-dlp.sh master          # bleeding edge
#
# Knobs (env overrides):
#   PYTHON            interpreter to build with (default: first python >=3.10
#                     found: python3.14/3.13/.../3.10, then `python3`).
...
# produces a single-arch binary (this host's arch, e.g. x86_64 or aarch64) named
# yt-dlp_linux. It is also dynamically linked against the host glibc, so build on
# the OLDEST distro you need to support — a bundle built on a newer glibc won't
# start on an older one. The result is unsigned; fine to run locally.
#
...
# yt-dlp needs >=3.10. Most current distros ship a new enough python3, but honor
# $PYTHON, else probe the usual versioned names, else fall back to a `python3`
# that passes the version gate.
...
git_bin=""
for cand in "${GIT:-}" git /usr/bin/git /usr/local/bin/git; do
    [[ -n "$cand" ]] || continue
...
if [[ -z "$git_bin" ]]; then
    ...
    echo "Install git (Debian/Ubuntu: 'sudo apt install git'; Fedora/RHEL:" >&2
    echo "'sudo dnf install git'), or point at one with GIT=/path/to/git $0 ${VERSION}." >&2
    exit 1
fi
if [[ -z "$python" ]]; then
    echo "Error: no Python >=3.10 found (yt-dlp requires it)." >&2
    echo "Install one (Debian/Ubuntu: 'sudo apt install python3 python3-venv';" >&2
    echo "Fedora/RHEL: 'sudo dnf install python3'), or point at it with" >&2
    echo "PYTHON=/path/to/python3.xx $0 ${VERSION}." >&2
    exit 1
fi
...
# On Debian/Ubuntu `python3 -m venv` needs the python3-venv package; surface a
# clear fix if it's missing rather than a cryptic ensurepip error.
if [[ ! -x "$venv_dir/bin/python" ]]; then
    log "Creating virtualenv at ${venv_dir}"
    if ! "$python" -m venv "$venv_dir"; then
        echo "Error: failed to create a virtualenv." >&2
        echo "On Debian/Ubuntu install the venv module: 'sudo apt install python3-venv'" >&2
        echo "(or the version-matched 'python3.NN-venv')." >&2
        exit 1
    fi
fi
...
# --- locate + collect --------------------------------------------------------
# The dist name is platform-specific (yt-dlp_linux_<arch> / yt-dlp_macos /
