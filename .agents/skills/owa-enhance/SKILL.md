---
name: owa-enhance
description: Research and rank improvements across Open Worship App security, performance, reliability, UI, development workflow, code health and documentation. Use for app-wide improvement or audit requests. Research is read-only except for its report; apply changes only when the user requests them. Route chatbot, MCP and AI Chat work to their dedicated skills.
---

## Codex usage

Invoke as `$owa-enhance` followed by the request or options.
