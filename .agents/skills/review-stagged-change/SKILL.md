...
description: Review only the Git index and staged diff for performance, correctness, reuse, simplification, conventions, tests, security and documentation drift. Use for staged-change or pre-commit review requests. Inspect index versions, verify findings and report concrete fixes. Apply fixes to the worktree only when requested.
---

## Codex usage

Invoke as `$review-stagged-change` followed by the request or options.
...
Arguments: [optional: path/glob to narrow the review, "perf"/"correctness"/"conventions" to weight a dimension, or "fix" to apply findings after reporting]
