...
description: Review only unstaged changes and non-ignored untracked files for performance, correctness, conventions, security, documentation drift and unfinished work. Use for work-in-progress or before-staging reviews. The index is the baseline for partially staged files. Never stage, commit or stash; apply fixes only when requested.
---

## Codex usage

Invoke as `$review-unstagged-change` followed by the request or options.
