...
description: Improve, debug and harden the Open Worship App AI Chat window, its external chat sites, tabs, sign-in and sandboxed webview guests. Use for AI Chat window requests, including site loading or bot-check failures. The in-app help assistant belongs to owa-enhance-chatbot.
---

## Codex usage

Invoke as `$owa-enhance-aichat` followed by the request or options.
...
Arguments: [security | sites | tabs | perf | audit | full — or a plain description of the change]
