...
description: Improve, audit and harden the Open Worship App owa-devtools MCP server, tool schemas, firewall, transports and developer experience. Use for MCP tool behavior, safety, token cost or server changes. Measure before and after, verify against the running app, and preserve the developer and in-app chatbot interfaces.
---

## Codex usage

Invoke as `$owa-enhance-mcp` followed by the request or options.
